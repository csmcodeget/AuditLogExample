...
public class DeleteLogHelper {
    private static final Logger logger = LoggerFactory.getLogger(DeleteLogHelper.class);

    public static <T> String getData(T entity) {
        if (entity == null) return "";
        try {
            ChangeResult result = EntityChangeProcessor.compareForDelete(entity);
            return ChangeResultFormatter.format(result, OperationType.DELETE);
        } catch (Exception e) {
            logger.error("获取删除变更数据失败: {}", e.getMessage(), e);
            return "";
        }
    }
    ...
    public static <T> String getData(T entity, Set<String> excludeFields) {
        ...
        try {
            ChangeResult result = EntityChangeProcessor.compareForDelete(entity);
            return ChangeResultFormatter.formatWithExcludes(result, OperationType.DELETE, excludeFields);
        } catch (Exception e) {
            logger.error("获取删除变更数据失败: {}", e.getMessage(), e);
            return "";
        }
    ...
    public static <T> String getDataWithInclude(T entity, Set<String> includeFields) {
        ...
        try {
            ChangeResult result = EntityChangeProcessor.compareForDelete(entity);
            return ChangeResultFormatter.formatWithIncludes(result, OperationType.DELETE, includeFields);
        } catch (Exception e) {
            logger.error("获取删除变更数据失败: {}", e.getMessage(), e);
            return "";
        }
